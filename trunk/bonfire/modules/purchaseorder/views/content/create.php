		...
		<select id="purchaserequisition_ids" name="purchaserequisition_ids" multiple="multiple">
			<?php if(isset($purchaserequisition_options)){ ?>
				<?php foreach($purchaserequisition_options as $value => $option){ ?>
					<option value="<?php echo $value ?>" <?php echo set_select('purchaserequisition_ids', $value) ?>><?php echo $option ?></option>	
				<?php } ?>
			<?php } ?>
		</select>
</div>
...
<div class="text-right">
	<br/>
	<input type="submit" name="submit" value="Create PurchaseOrder" /> or <?php echo anchor(SITE_AREA .'/content/purchaseorder', lang('purchaseorder_cancel')); ?>
</div>
<?php echo form_close(); ?>
